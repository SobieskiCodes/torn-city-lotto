...
class timedGiveaway(commands.Cog):
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if user.id not in self.bot.addedids and user.id != self.bot.user.id:
            await reaction.message.channel.send(f"{user.mention}, please add your torn ID using $addid <id>!",
                                                delete_after=3)
    ...
    async def giveaway(self, ctx, length, winners, prize):
        e = discord.Embed(
            title=f"{self.bot.party}{ctx.message.author.name} just started a giveaway!{self.bot.party}",
            description=f"The prize: {prize} \nGiveaway ends in {length // 60}m\nNumber of winners: {winners}\n"
                        f"React with {self.bot.party} to enter!",
        ...
        if len(users) > 2:
            ...
            new_e = discord.Embed(
                title=f"{self.bot.party}{ctx.message.author.name}'s giveaway has ended!{self.bot.party}",
            ...
            another_e = discord.Embed(
                description=f"{self.bot.party}{ctx.message.author.name}'s giveaway has ended! \n"
            ...
            author_embed = discord.Embed(description=f"Hey, the giveaway ended! [link]({the_giveaway.jump_url})")
    ...
    async def gstart(self, ctx, time: str = None, winners: str = None, *, prize: str = None):
        if not time or not winners or not prize:
            e = discord.Embed(colour=discord.Colour(0xbf2003),
                                description=f"{self.bot.no} {ctx.author.name}, "
    ...
    async def giveawaychan(self,  ctx, chan: str = None):
        ...
        if not chan or not ctx.message.channel_mentions:
            e = discord.Embed(colour=discord.Colour(0xbf2003),
                              description=f"{self.bot.no} {ctx.author.name}, please provide a channel to set!")
        ...
        if ctx.message.channel_mentions:
            if len(ctx.message.channel_mentions) == 1:
                ...
                e = discord.Embed(colour=discord.Colour(0x03bd33),
                                  description=f"{self.bot.yes} Giveaway channel has been updated to {the_chan.name}")
                await ctx.send(embed=e, delete_after=3)
                await ctx.message.delete()
                ...
                e = discord.Embed(colour=discord.Colour(0xbf2003),
                                  description=f"{self.bot.no} {ctx.author.name}, please provide only a single channel to set!")
                await ctx.send(embed=e, delete_after=3)
                await ctx.message.delete()
